// VOD pipeline mock data — one simulated trading day (2026-05-19) +
// derived beat list + per-subsystem pipeline status. One useVodMock()
// at the top-level feeds the four artboards.
...
function officeDisplay(id) {
  const n = officeName(id);
  const [a, b] = n.split('_');
  return `${a[0].toUpperCase() + a.slice(1)} ${b[0].toUpperCase() + b.slice(1)}`;
}
function officeInitials(id) {
  const n = officeName(id);
  const [a, b] = n.split('_');
  return (a[0] + b[0]).toUpperCase();
}

const VOD_STRATEGIES = ['momentum', 'lstm', 'llm'];
const VOD_STRATEGY_LABEL = { momentum: 'momentum_sma20', lstm: 'lstm_v1', llm: 'lstm_llm_v1' };
const VOD_STRATEGY_SHORT = { momentum: 'MOM', lstm: 'LSTM', llm: 'LSTM+LLM' };
const VOD_STRATEGY_HUE = { momentum: 32, lstm: 200, llm: 280 };
const VOD_RANKS = ['intern', 'junior', 'senior', 'principal'];

// Deterministic PRNG so the mock day is reproducible.
...
// String-keyed variant: hashes a string into a seed, returns rng().
function seededRng2(key) {
  let s = 0;
...
// --- Day metadata ---
const SESSION_DATE = '2026-05-19';
const SESSION_LABEL = 'Tue, May 19 · session s_2026-05-19_a3f2';
const SESSION_ID = 's_2026-05-19_a3f2';
const SESSION_EP_NUMBER = 47;
...
// --- Agents for this day's roster ---
function makeVodAgents() {
  const agents = [];
  for (let i = 0; i < 100; i++) {
    const r = seededRng(i + 47);
    const strat = VOD_STRATEGIES[i % 3];
    ...
    agents.push({
      ...
      strategy: strat,
      rank: VOD_RANKS[rankIdx],
      equity, pnl, pnlPct: pnl / 10,
      sparkline: spark,
      trades, wins: Math.floor(trades * (0.4 + r() * 0.45)),
    });
  }
  return agents;
}

const VOD_AGENTS = makeVodAgents();

// --- The beat list. Curated to feel like a real day's story. ---
// Times are HH:MM in ET. Score 0-1. Kind matches the beat detector
// taxonomy from docs/vod-pivot.md.
const BEATS = [
  {
...
// --- Pipeline subsystems (the 10 from docs/vod-pivot.md) ---
const PIPELINE = [
  {
    id: 'session',
    label: 'Session runner',
    cmd: 'tradefarm.session.run',
    status: 'done',
    started: '16:00:04', finished: '16:01:12', durationSec: 68,
  ...
  },
  {
    id: 'beats',
    label: 'Beat detector',
    cmd: 'tradefarm.session.beats',
    status: 'done',
    started: '16:01:12', finished: '16:01:18', durationSec: 6,
    output: 'beats.json',
    outputSize: '12.4 KB',
  ...
  },
  {
    ...
    cmd: 'tradefarm.render.headless',
    status: 'running',
    started: '16:01:19', finished: null, durationSec: 142,
    progress: 0.62,
    progressLabel: '8/13 clips · ETA 1m 28s',
    output: 'clips/*.mp4',
    outputSize: '— (8/13 in progress)',
  ...
  {
    id: 'stitch',
    label: 'ffmpeg stitcher',
    cmd: 'tradefarm.render.stitch',
    status: 'queued',
    started: null, finished: null, durationSec: null,
    output: 'silent_reel.mp4',
    outputSize: null,
...
// --- Day account/PnL roll-up ---
function makeDaySummary(agents) {
...
const DAY_SUMMARY = makeDaySummary(VOD_AGENTS);

// --- Caption styles, episode lengths, etc (for Tweaks) ---
const CAPTION_STYLES = ['minimal', 'kinetic', 'news-chyron', 'serif'];
const COLOR_THEMES = ['studio dark', 'studio light', 'amber CRT'];

// --- Hook ---
function useVodMock() {
  // Light "render is progressing" state so the pipeline view animates.
  const [renderTick, setRenderTick] = React.useState(0);
  React.useEffect(() => {
    const id = setInterval(() => setRenderTick(t => t + 1), 800);
    return () => clearInterval(id);
  }, []);

  // Beat selection state (operator can flip them on/off in Beat Picker).
  const [selectedBeats, setSelectedBeats] = React.useState(
    () => new Set(BEATS.filter(b => b.selected).map(b => b.id))
  );
  const toggleBeat = id => setSelectedBeats(prev => {
    const next = new Set(prev);
    if (next.has(id)) next.delete(id); else next.add(id);
    return next;
  });

  const totalDuration = BEATS
    .filter(b => selectedBeats.has(b.id))
    .reduce((s, b) => s + b.duration, 0);

  // Synthesize a "current render progress" from renderTick so the
  // headless-renderer card looks alive even while idle in the mock.
...
Object.assign(window, {
  useVodMock,
  officeName, officeDisplay, officeInitials,
  seededRng, seededRng2,
  VOD_AGENTS, VOD_STRATEGIES, VOD_STRATEGY_LABEL, VOD_STRATEGY_SHORT, VOD_STRATEGY_HUE,
  VOD_RANKS, BEATS, BEAT_KIND_META, PIPELINE, DAY_SUMMARY,
  SESSION_DATE, SESSION_LABEL, SESSION_ID, SESSION_EP_NUMBER,
  CAPTION_STYLES, COLOR_THEMES,
  // Aliases so shared.jsx's stratColor() (which reads STRATEGY_HUE) works
  // when only the VOD mocks are loaded (e.g. the Dashboard).
  STRATEGY_HUE: VOD_STRATEGY_HUE,
});
