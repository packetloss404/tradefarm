...
def _write_session(
    ...
    *,
    ...
    for bid in rendered_beat_ids or []:
        (sdir / "clips" / f"{bid}.json").write_text(json.dumps(_sidecar(bid, duration_ms=30000)))
...
def test_description_includes_summary_then_boilerplate_then_chapters():
    beats = [
        _beat("b_recap", kind="recap", sub="+1.84% pool · best since Apr 24"),
        _beat("b1"),
        _beat("b2"),
        _beat("b3"),
    ]
    ...
    desc = build_description(
        beats=beats,
        script={"episode_title": "Mei takes #1"},
        manifest=None,
        chapters=chapters,
        session_id="s_x",
    )
