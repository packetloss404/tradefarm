    ...
    *,
    ...
    (sdir / "beats.json").write_text(json.dumps([
        {"id": bid, "t": f"2026-05-21T14:0{i}:00+00:00",
         "kind": "big_fill", "headline": "h", "sub": "s",
    ...
    (sdir / "vo" / "index.json").write_text(json.dumps({
        ...
        "sample_rate": 22050, "lines": vo_lines,
    }))
    return sdir
...
def test_plan_mix_threads_multiple_vo_lines_per_beat(tmp_path: Path, monkeypatch):
    ...
    expected = [round(x, 3) for x in (
        0.5, 0.5 + 2.0 + 0.1, 0.5 + 2.0 + 0.1 + 3.0 + 0.1,
    )]
    assert o == expected
...
def test_plan_mix_skips_beats_without_sidecar(tmp_path: Path, monkeypatch):
    ...
    plan = plan_mix(
        session_id="s_skip", sessions_dir=tmp_path, music_path=None,
    )
...
def test_plan_mix_with_no_vo_returns_empty_lines(tmp_path: Path, monkeypatch):
    ...
    plan = plan_mix(
        session_id="s_novo", sessions_dir=tmp_path, music_path=None,
    )
...
def _make_plan(*, vo_lines: list[VoLine] | None = None,
               music_path: Path | None = None,
               reel_dur: float = 60.0) -> MixPlan:
    return MixPlan(
        session_id="s_x",
...
def test_build_mix_command_with_vo_only_delays_each_line():
    vo = [
        VoLine(beat_id="b0", line_idx=0,
               wav=Path("vo/b0_00.wav"), duration_sec=2.0, onset_sec=0.5),
        VoLine(beat_id="b1", line_idx=0,
               wav=Path("vo/b1_00.wav"), duration_sec=2.0, onset_sec=20.0),
    ]
...
def test_build_mix_command_with_music_includes_loop_and_duck():
    vo = [VoLine(beat_id="b0", line_idx=0,
                  wav=Path("vo/b0_00.wav"), duration_sec=2.0, onset_sec=0.5)]
...
def test_mix_session_dry_run_skips_ffmpeg(tmp_path: Path, monkeypatch):
    ...
    _write_session(
        tmp_path, "s_dry",
        beats_ids=["b0"], clip_durations_sec=[10.0],
        vo_per_beat={"b0": [2.0]},
    )
...
def test_integration_mix_silent_reel_with_vo(tmp_path: Path):
    ...
    subprocess.run([
        ...
        "-an", str(sdir / "silent_reel.mp4"),
    ], check=True)
    # Beats + sidecar pointing at a 6s clip starting at 0.
    (sdir / "clips").mkdir()
    (sdir / "beats.json").write_text(json.dumps([
        {"id": "b0", "t": "2026-05-21T14:00:00+00:00", "kind": "open",
         "headline": "h", "sub": "s", "duration_sec": 6}
    ]))
    (sdir / "clips" / "b0.json").write_text(json.dumps({
        "beat_id": "b0", "kind": "open", "scene": "hero",
        "at": "x", "until": "x", "duration_ms": 6000,
        "scene_ready_at_ms": 0, "elapsed_ms": 6000,
        "viewport": [320, 180], "url": "x",
        "clip": "b0.webm", "captured_at": "x",
    }))
    # 1s synthetic VO wav.
    (sdir / "vo").mkdir()
    ...
    (sdir / "vo" / "index.json").write_text(json.dumps({
        "session_id": "s_int", "provider": "silence", "voice": "Test",
        "sample_rate": 22050,
        "lines": [{"beat_id": "b0", "line_idx": 0, "text": "test",
                    "wav": "b0_00.wav", "duration_sec": 1.0,
                    "provider": "silence", "voice": "Test"}],
    }))
